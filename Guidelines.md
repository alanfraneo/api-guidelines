...
## 1. Abstract
The REST API Guidelines, as a design principle, encourages application developers to have resources accessible to them via a RESTful HTTP interface.
...
## 3. Introduction
One of the goals of these guidelines is to ensure REST APIs can be easily and consistently consumed by any client with basic HTTP support.
...
- Make accessing Services via REST interfaces easy for all application developers.
- Allow developers to leverage the prior work of other developers to implement, test and document REST endpoints defined consistently.
- Allow for partners to use these guidelines for their implementation of extensions using our APIs.
...
These guidelines are applicable to any REST API exposed publicly.
...
Consistency is valuable to not only external customers but also internal API consumers, and these guidelines offer best practices useful for any situation.

### 4.2. Guidelines for versioning of services
...
## 5. Taxonomy
As part of the REST API Guidelines, services must be familiar with the taxonomy defined below:

### 5.1. Errors
Errors are defined as a client passing invalid data to the service and the service _correctly_  rejecting that data.
...
### 5.2. Faults
Faults are defined as the service failing to correctly return in response to a valid client request.
These are generally "5xx" HTTP error codes.

Faults _do_ contribute to the overall API availability.

Calls that fail due to rate limiting or quota must not count as faults.
...
```
https://api.contoso.com/EWS/OData/Users('jdoe@contoso.com')/Folders('AAMkADdiYzI1MjUzLTk4MjQtNDQ1Yy05YjJkLWNlMzMzYmIzNTY0MwAuAAAAAACzMsPHYH6HQoSwfdpDx-2bAQCXhUk6PC1dS7AERFluCgBfAAABo58UAAA=')
```
...
As an example, imagine a service that allows creation of projects, which will be named by the service:

```http
POST http://dataprepapi.zoho.com/v1/workspace/account1/projects
```
...
```http
201 Created
Location: http://dataprepapi.zoho.com/v1/workspace/account1/projects/123
```

Where "123" is the newly created project's ID.

Services may also return the full metadata for the created item in the response based on a header configuration that is specified in their contract -- tdb.
...
REST API Guidelines compliant APIs should support PATCH.

#### 7.4.3. Creating resources via PATCH (UPSERT semantics)
Under UPSERT semantics, a PATCH call to a nonexistent resource is handled by the server as a "create," and a PATCH call to an existing resource is handled as an "update."
APIs that allow callers to specify ID values on create should support UPSERT semantics, and those that do must support creating resources using PATCH.
...
To ensure that an update request is not treated as a create or vice versa, the client may specify precondition HTTP headers in the request.
The service must not treat a PATCH request as an insert if it contains an _If-Match_ header and must not treat a PATCH request as an update if it contains an _If-None-Match_ header with a value of "*".
...
```http
Link: <https://help.zoho.com/dataprep/api/projects>; rel="help"
```

Where https://help.zoho.com/dataprep/api/projects is the URL to a documentation resource.

For examples on use of OPTIONS, see [preflighting CORS cross-domain calls][cors-preflight].

### 7.5. Standard request headers
The table of request headers below should be used by REST API Guidelines services.
...
For non-success conditions, developers should be able to write one piece of code that handles errors consistently across different REST API Guidelines services.
...
This object must have a status key with value "failed." The object should contain the error(s) in an array of objects with a key named: errors.

Each object in this array must contain name/value pairs with the names "status_code" and "message," and it may contain name/value pairs with the names "target" and "type"

The value for the "code" name/value pair is a language-independent string.
Its value is a service-defined error code that should be human-readable.
This code serves as a more specific indicator of the error than the HTTP error code specified in the response. Services should have a relatively small number (about 20) of possible values for "code," and all clients must be capable of handling all of them.
...
## 8. CORS
Services compliant with the REST API Guidelines must support [CORS (Cross Origin Resource Sharing)][cors].
...
- Services may add additional metadata to the "@removed" node, such as a reason for removal, or a "removed at" timestamp. We recommend teams coordinate with the REST API Guidelines Working Group on extensions to help maintain consistency.
...
Some clear examples of breaking changes:
...
Services that allow client defined Web Hook Callback URLs should not transmit data over HTTP. This is because information can be inadvertently exposed via client, network, server logs and other mechanisms.
...
The features are:
- Key Addressing in a collection, such as: `https://dataprepapi.zoho.com/v1.0/users/user1@contoso.com`
- Filtering a collection by a property value, such as: `https://dataprepapi.zoho.com/v1.0/users?$filter=name eq 'david'`
- Filtering a collection by range, such as: `http://dataprepapi.zoho.com/v1.0/users?$filter=hireDate ge 2014-01-01 and hireDate le 2014-12-31`
- Client-driven pagination via $top and $skip, such as: `http://dataprepapi.zoho.com/v1.0/users?$top=5&$skip=2`
- Sorting by $orderBy, such as: `https://dataprepapi.zoho.com/v1.0/users?$orderBy=name desc`
- Providing $delta tokens, such as: `https://dataprepapi.zoho.com/v1.0/users?$delta`
...
Unless a more specific error status is appropriate for the given request, services should return "400 Bad Request" and an error payload conforming to the error response guidance provided in the REST API Guidelines.
